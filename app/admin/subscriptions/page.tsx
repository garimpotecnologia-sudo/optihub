...
const statusColors: Record<string, string> = {
  ...
  EXPIRED: "bg-bg-deep text-text-muted",
};
...
export default function AdminSubscriptionsPage() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      const params = new URLSearchParams({ type: "main" });
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold font-[var(--font-heading)] tracking-tight">Assinaturas</h1>
        <p className="text-text-muted text-sm mt-1">Assinaturas de planos (Pro/Rede).</p>
      </div>

      <div className="flex gap-2">
        {["", "ACTIVE", "PENDING", "OVERDUE", "CANCELLED"].map((s) => (
          <button key={s} onClick={() => setStatusFilter(s)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
              statusFilter === s ? "bg-accent-violet/15 text-accent-violet border border-accent-violet/20" : "bg-bg-card border border-border text-text-muted"
            }`}>
            {s || "Todos"}
          </button>
        ))}
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="text-left text-[11px] text-text-muted font-medium uppercase tracking-wider px-5 py-3">Próximo</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-5 py-8 text-center text-sm text-text-muted">Nenhuma assinatura encontrada</td></tr>
            ) : subs.map((s) => (
              <tr key={s.id} className="border-b border-border hover:bg-bg-card-hover/50 transition-colors">
                ...
                <td className="px-5 py-3.5 text-xs text-text-muted">{s.next_due_date || "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
