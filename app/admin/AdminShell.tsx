...
export default function AdminShell({ userName, children }: { userName: string; children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 border-r border-[rgba(139,92,246,0.08)] flex flex-col transform transition-transform duration-200 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`} style={{ background: "linear-gradient(180deg, rgba(12,26,20,0.97) 0%, rgba(10,21,16,0.99) 100%)", backdropFilter: "blur(24px)" }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 flex items-center justify-between px-4 sm:px-6 border-b border-[rgba(139,92,246,0.06)]" style={{ background: "rgba(12,26,20,0.6)", backdropFilter: "blur(24px)" }}>
          ...
          <span className="text-sm text-text-muted hidden lg:block">Painel Administrativo</span>
          <div />
        </header>
