...
export default function LinktreeView({ linktree }: { linktree: Linktree }) {
  const { title, bio, cover_image, logo, primary_color, secondary_color, text_color, buttons } = linktree;
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: secondary_color }}>
      ...
      {/* Content */}
      <div className={`flex-1 flex flex-col items-center px-6 pb-12 ${cover_image ? "-mt-16" : "pt-12"}`}>
        {/* Logo */}
        {logo && (
          <div className="w-24 h-24 rounded-full border-4 overflow-hidden mb-4 shadow-lg" style={{ borderColor: primary_color }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
